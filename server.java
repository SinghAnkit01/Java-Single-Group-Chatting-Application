...
public class server implements ActionListener {
    ...
    server() {
        ...
        p1.add(icon);
         ...
         heading = new JLabel("Rahul Singh");
        heading.setBounds(85, 15, 100, 25);
        ...
        send1.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
             ...
             // sending message using DataOutputStream dout with writeUTF() protocol on arrow click from server to client
    ...
    public static void main(String[] args) {
        new server();
        // creating server in main class and while creating server we can get an error
        // or exception for that we have to take try-catch block
        try {
            ServerSocket skt = new ServerSocket(6001);
            // i have to run my server infinitely to accept messages from client and dispaly
            // it to client
            // server get started at 6001 port no.
            // System.out.println("server is running on port " + skt);
            while (true) {
                // while(true) use to run loop infinitely.
                // accepting messages and storing it into socket
                Socket s = skt.accept();
                // we can send and receive messages through.
                DataInputStream din = new DataInputStream(s.getInputStream());
                // use to receive messages
                // datainputstream comes under io package
                dout = new DataOutputStream(s.getOutputStream());
                // use to send messages
                while (true) {
                    // protocol used to receive and send data is readUTF() and writeUTF()
                    String msg = din.readUTF();
                    // to read messages with the help of DataInputStream.readUTF();
                    // client message to -> server
                    // after reading message we need to display it on frame
                    JPanel panel = formatLabel(msg);
                    JPanel left = new JPanel(new BorderLayout());
                    left.add(panel, BorderLayout.LINE_START);
                    vertical.add(left);
                    // make vertical static to global variable Box vertical =
                    // Box.createVerticalBox();
                    f.validate();
                    // validate() is a non-static method so it cant call staic jframe for that we
                    // have to make jframe static so, remove extends jframe and make object of jframe class globally
                    // static jframe frame = new JFrame();
                    // now,i have make the frame static now i can call methods using object of jframe class as i had done in f.validate().
